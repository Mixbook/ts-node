...
function register(options) {
    ...
    var typeCheck = !!(options.typeCheck == null ? DEFAULTS.typeCheck : options.typeCheck);
    ...
    for (var _i = 0, _a = config.fileNames; _i < _a.length; _i++) {
        var fileName = _a[_i];
        var regexpContent = ['.d.ts'].concat(extensions).map(function (e) { return e.replace(/\./g, '\\.'); }).join('|');
        var regexp = new RegExp('(' + regexpContent + ')$');
